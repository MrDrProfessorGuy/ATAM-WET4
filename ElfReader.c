...
#define NextRelaSym(relasym, size) ((Elf64_Rela*) ((char*)relasym + size))
...
int readSymtab(const ElfFile elf_file, Elf64_Shdr symtab_sh, char* sym_name, Elf64_Sym* func_sym){
    ...
    for (Elf64_Xword index = 0; index < sym_num; index++) {
        ...
        if (symbol->st_name != 0 && strcmp(sym_str, sym_name) == 0){
            *func_sym = *symbol;
            return SYM_FOUND;
        }
...
int cmp_sym_name(const ElfFile elf_file, Elf64_Shdr symtab_sh, char* sym_name, Elf64_Sym func_sym){
    ...
    char* sym_str = FileOffset(void*, elf_file,(sh_strtab.sh_offset+func_sym.st_name));
    if (func_sym.st_name != 0 && strcmp(sym_str, sym_name) == 0){
        return SYM_FOUND;
    }
    return NAME_NOT_FOUND;
...
int readRelaSym(const ElfFile elf_file, Elf64_Shdr sh_rela, char* sym_name, Elf64_Addr* func_GOT_address){
    ...
    Elf64_Rela* func_plt = FileOffset(Elf64_Rela*, elf_file, sh_rela.sh_offset);
    for (Elf64_Xword index = 1; index <= sym_num; index++) {
        
        func_sym = FileOffset(Elf64_Sym*, elf_file,(sh_dynsym.sh_offset+ sh_dynsym.sh_entsize*ELF64_R_SYM(func_plt->r_info)));

        if (cmp_sym_name(elf_file,sh_dynsym,sym_name,*func_sym) == SYM_FOUND){
            //*func_GOT_address = sh_rela.sh_addr + index*sym_size;
            *func_GOT_address = func_plt->r_offset;
            return SYM_FOUND;
        }
        func_plt = NextRelaSym(func_plt, sym_size);
    }
...
int readDynSym(const ElfFile elf_file, Elf64_Shdr sh_rela, char* sym_name, Elf64_Addr* func_address){
    ...
    for (Elf64_Xword index = 1; index <= sym_num; index++) {
        ...
        if (cmp_sym_name(elf_file,sh_dynsym,sym_name,*func_sym) == SYM_FOUND){
            *func_address = sh_rela.sh_addr + index*sym_size;
            return SYM_FOUND;
        }
        func_plt = NextRelaSym(func_plt, sym_size);
    }
